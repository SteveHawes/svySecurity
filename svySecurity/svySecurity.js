...
>>>>>>> 6172b702871ecdaf22cb9d48b465f22e6e2a00f6
}
 ...
 * @param {Boolean} [checkAdditionalEmail]
 ...
 * @AllowToRunInFind
 */
function getUser(userNameOrId, tenantNameOrId, checkAdditionalEmail) {
  ...
  if (userNameOrId instanceof UUID || application.getUUID(uuidString) == uuidString) {
    ...
    var or = qry.or;
    or.add(qry.columns.user_name.eq(userNameOrId));
    if (checkAdditionalEmail)
      or.add(qry.columns.additional_email.eq(userNameOrId))
    qry.where.add(or);
  }
...
function loginWithToken(namespace) {
  ...
  var user = getUser(payload.user, payload.tenant, true);
  if (!user) {
...
function logFailedLogin(userName, tenantName, error) {
  ...
  var user = getUser(userName, tenantName, true);
  if (user) {
