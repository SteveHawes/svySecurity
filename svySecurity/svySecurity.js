...
/**
 * Creates a permission and also syncs to the internal Servoy security
 * @properties={typeid:24,uuid:"423080C2-56FC-4174-BBF1-FB45D72B0148"}
 */
function createPermissions(pmName,pmDisplayName){
	var permissionFS = datasources.db.svy_security.permissions.getFoundSet();
	permissionFS.newRecord();
	permissionFS.display_name = pmDisplayName;
	permissionFS.permission_name = pmName;
	databaseManager.saveData(permissionFS);		
	if(!security.createGroup(pmName)) {
		logWarning(utils.stringFormat('Could not create "%1$s" internal group',[pmName]))
	}
...
function syncPermissions(forcePermissionRemoval) {
	...
	if (forcePermissionRemoval) {
		// look for removed permissions and remove from internal security
		var qry = datasources.db.svy_security.permissions.createSelect();
		...
		//sync with internal group		
		permissionFS.loadAllRecords();
		for (var j = 1; j <= permissionFS.getSize(); j++) {
			var r = permissionFS.getRecord(j);
			if(!security.createGroup(r.display_name)) {
				logWarning(utils.stringFormat('Could not create "%1$s" internal group',[record.permission_name]))
			}
		}
	}
}
